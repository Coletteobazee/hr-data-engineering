...
def run(filename: str) -> tuple[float, float, float]:
    """
    ...
    plt.title(f"Heart Rate Data for {filename}")
    plt.xlabel("Time")
    plt.ylabel("Heart Rate")

    image_filename = f"images/{filename.split('/')[-1].replace('.txt', '_plot.png')}"
